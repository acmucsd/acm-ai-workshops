    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        def update_probability_matrix(row, col, action):
            ...
            terminated = bytes(newletter) in b"G" # only terminate when at goal

            if newletter == b"G":
                reward = 1 # positive reward, end the episode
            elif newletter == b"H":
                reward = -1 # negative reward, but do not terminate the episode
            else:
                reward = 0 # no reward, continue the episode
